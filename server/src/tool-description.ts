...
Strict Rules:
- NO web browsing. Use ONLY this tool to find items.
...
- When the user mentions a style, era, aesthetic, vibe, reference, or asks for an outfit/look/styling help, READ the styling-instructions resource (resource:///styling-instructions) BEFORE building queries. It contains the persona, the style cheat sheet (keywords + priority brands per aesthetic: Y2K, Parisian, Cottagecore, Quiet Luxury, Boho, Grunge, Scandi, Old Money, etc.), and tone guidelines. Skipping it leads to weak, generic queries and missed brand opportunities.
...
3. For each item, decide type, color, material, silhouette, brands (match aesthetics to the Style Cheat Sheet when relevant), and whether any filter applies.
